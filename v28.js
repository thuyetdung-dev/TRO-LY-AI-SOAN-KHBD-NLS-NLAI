...
async function testAI(){const b=$('v28TestAiBtn'),provider=$('aiProvider')?.value;b.disabled=true;b.textContent='Đang kiểm tra…';try{if(provider==='gemini'){$('scanModels')?.click();note('Đang kiểm tra Gemini. Xem kết quả trong mục Nguồn AI.');return}const r=await fetch('/api/openai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:'Chỉ trả lời đúng một từ: OK',files:[]})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Kết nối thất bại');note('Kết nối OpenAI hoạt động tốt.')}catch(e){note('Kiểm tra AI: '+e.message)}finally{b.disabled=false;b.textContent='Kiểm tra kết nối AI'}}
function preflight(){
