...
function maxTokensFor(fullName){const lim=OUTPUT_LIMITS.get(fullName);return Math.min(lim||8192,32768)}
...
2) Bám sát đúng cấu trúc KHBD theo Phụ lục IV, Công văn số 5512/BGDĐT-GDTrH ngày 18/12/2020 dưới đây — không tự đổi tên đề mục cấp I/II/III, không tự đổi tên/số lượng 4 hoạt động (chỉ được thêm tên riêng thể hiện nội dung bài vào sau tên hoạt động gốc):
${KHBD_TEMPLATE_5512}
Sau Hoạt động 4 (Vận dụng), có thể thêm mục "Điều chỉnh sau bài dạy" (để trống hoặc ghi "..." vì đây là phần GV tự ghi sau khi dạy thực tế).
3) Trong mỗi hoạt động, mục d) Tổ chức thực hiện tuyệt đối không dùng bảng Markdown. Bắt buộc xuất một khối mã lessonflow chứa JSON hợp lệ. Hoạt động 1, 2, 3 có đúng 4 rows (Giao nhiệm vụ → Thực hiện → Báo cáo, thảo luận → Kết luận, nhận định). Riêng Hoạt động 4 (Vận dụng), nếu giao cho học sinh thực hiện ngoài giờ học trên lớp thì chỉ cần 2–3 rows cho phần diễn ra tại lớp (giao nhiệm vụ, hướng dẫn cách nộp và tiêu chí đánh giá) — không bịa thêm bước báo cáo, kết luận nếu chúng thực sự diễn ra ở buổi học sau; nếu tổ chức trọn vẹn trên lớp thì vẫn dùng đủ 4 rows. Mỗi row dùng schema: {"step":"Bước ...","duration":3,"goalIds":["MT1"],"teacherActions":["GV ..."],"studentActions":["HS ..."],"product":["SP1: nội dung/đáp án cụ thể"],"assessment":["TC1: tiêu chí và cách thu thập minh chứng"],"competency":["Mã: hành vi quan sát được"],"sourceTag":"Theo nguồn: tên tệp/mục; hoặc AI đề xuất"}. Khối đầy đủ có dạng {"type":"lessonflow","rows":[...]}. duration là số phút nguyên dương; goalIds chỉ được dùng mã mục tiêu đã khai báo; sản phẩm phải có mã SP; tiêu chí phải có mã TC. Mỗi công thức trong JSON đặt giữa $...$ và gạch chéo ngược LaTeX viết thành hai gạch chéo ngược để JSON hợp lệ.
...
8) THỜI LƯỢNG: Mỗi row lessonflow có duration. Tổng thời lượng toàn bài phải đúng ${v.periods?Number(v.periods)*45:'số tiết đề xuất × 45'} phút; cuối Tiến trình ghi "Tổng thời lượng: ... phút". Nếu giao vận dụng ngoài giờ, duration ghi thời gian giao nhiệm vụ trên lớp, không cộng thời gian HS làm ở nhà.
...
11) Câu hỏi phải chính xác; tự giải và kiểm tra đáp án hai lần. Không sao chép máy móc giáo án tham khảo; không bịa dữ kiện từ nguồn. Đầu ra bằng Markdown, không mở đầu xã giao, phong cách ${$('style').value}.`}
